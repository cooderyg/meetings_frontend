...
const data: Payment[] = Array.from({ length: 100 }, (_, i) => {
   return {
      id: String(i + 1),
      title: '모바일 앱 개선 - 주요 이탈 구간 분석 결과 공유',
      createAt: getRandomDate(),
      participant: ['이정민', '김혜림', '강영구', '류원희', '인한별'],
      author: '이정민',
...
export const columns: ColumnDef<Payment>[] = [
   ...
   },
   {
      ...
      header: '참석자',
      cell: ({ row }) => {
         const participant: string[] = row.getValue('participant');
         const participantCount = participant.length;
         const displayCount = participantCount >= 99 ? '99+' : participantCount.toString();
               ...
               <div className="flex -space-x-2 *:data-[slot=avatar]:ring-background *:data-[slot=avatar]:ring-2">
                  {participant.map(p => (
                     <Avatar className="w-6 h-6">
                        ...
                        <AvatarFallback>{p.charAt(0)}</AvatarFallback>
                     </Avatar>
                  ))}
               </div>
   ...
   },
   {
      ...
      header: '녹음 시간',
      cell: ({ row }) => <div className="capitalize">{row.getValue('recordingTime')}</div>,
   },
