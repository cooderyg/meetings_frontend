...
export function SharePopover() {
   ...
   const [shareLink, setShareLink] = useState('http://example.com/link/to/document');
   ...
   const handleCopyLink = () => {
      const textToCopy = isPasswordProtected ? `${shareLink} (Password: ${password})` : shareLink;
         ...
         <PopoverContent className="w-[423px] p-4">
            <div className="flex flex-col gap-4">
               <div className="space-y-1.5">
                  ...
                  <p className="text-muted-foreground text-sm">링크가 있으면 누구나 로그인 후 볼 수 있어요.</p>
               </div>
               ...
               {isPublic && (
                  <>
                     <div className="flex items-center justify-between">
                        <Label className="text-sm text-slate-950 font-medium">링크 만료일</Label>
                        <div className="flex flex-col items-end">
                           ...
                           {linkExpiration !== 'never' && (
                              <p className="text-xs text-muted-foreground mt-1">{getExpirationDate(linkExpiration)}</p>
                           )}
                        </div>
                     </div>
                     ...
                     <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                           <Label className="text-sm text-slate-950 font-medium">비밀번호</Label>
                           <span className="flex gap-2 items-center">
                              {isPasswordProtected ? (
                                 <>
                                    <span className="text-slate-900 font-medium">{password}</span>
                                    <RotateCw size={16} />
                                 </>
                              ) : (
                                 <span className="text-slate-500font-medium font-medium">{password}</span>
                              )}
                           </span>
                        </div>
                        <div className="flex items-center gap-2">
                     ...
                     <div className="flex items-center gap-2 pt-2">
                        <Input type="text" value={shareLink} readOnly className="flex-1 h-9 text-sm" />
                        <Button
