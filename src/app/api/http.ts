...
export const http = {
  ...
  post: function httpPost<Request, Response = unknown>(
    url: string,
    data?: Request
  ) {
    return axios.post<Response>(url, data).then((res) => res.data);
  },
  delete: function httpDelete<Request, Response = unknown>(
    url: string,
    data?: Request
  ) {
    return axios.delete<Response>(url, { data }).then((res) => res.data);
  },
  put: function httpPut<Request, Response = unknown>(
    url: string,
    data?: Request
  ) {
    return axios.put<Response>(url, data).then((res) => res.data);
  },
  patch: function httpPatch<Request, Response = unknown>(
    url: string,
    data?: Request
  ) {
